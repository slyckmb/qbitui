...
PRESET_FILE = Path(__file__).parent.parent / "config" / "silo-filter-presets.yml"
...
def check_auth_bypass(opener: urllib.request.OpenerDirector, api_url: str) -> bool:
    """Check if the API is accessible without login (e.g. localhost whitelist)."""
    try:
        # /api/v2/app/version is lightweight and accessible if auth is bypassed
        req = urllib.request.Request(f"{api_url}/api/v2/app/version", method="GET")
...
def qbit_login(opener: urllib.request.OpenerDirector, api_url: str, username: str, password: str) -> bool:
    ...
    try:
        ...
        # Check for connection reset (Errno 104) which often indicates an IP ban
        is_reset = isinstance(e, ConnectionResetError)
        if not is_reset and isinstance(e, urllib.error.URLError):
            reason = e.reason
            is_reset = isinstance(reason, ConnectionResetError) or (isinstance(reason, OSError) and reason.errno == 104)
        
        if is_reset:
            print(f"ERROR: Connection reset by peer ({api_url}). Your IP might be banned by qBittorrent due to failed login attempts.", file=sys.stderr)
        return False
    except Exception:
        return False
...
def mediainfo_table(paths: list[Path]) -> str:
    ...
    hdr_line = "SILO_MI_V2|" + "|".join(hdr for hdr, _, _, _, _ in COLS)
...
def main() -> int:
    ...
    parser.add_argument("--config", default=os.environ.get("SILO_CONFIG_FILE"), help="Path to request-cache.yml")
        ...
        while True:
            ...
            for key in events:
                ...
                if key == "?":
                    ...
                    help_lines = []
                    help_lines.append(f"{colors.CYAN_BOLD}SILO HELP{colors.RESET}")
                    help_lines.append("")
