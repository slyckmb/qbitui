...
while true; do
  ...
  if [[ "$DIRECT_MODE" -eq 1 ]]; then
    ...
    if ! _raw="$(_rt_fetch_json_direct 2>/dev/null)"; then
      ...
      RT_CACHE_AGE_S=""
      RT_ACTIVE_LEASES="0"
      read -r CHECKING ERROR DOWN STALLED_DL PAUSED_DL STOPPED_DL SEEDING_UP STALLED_UP UPLOADING STOPPED_UP TOTAL TOP_STATES <<<"$(jq -r '
        [
    ...
    if ! SUMMARY_JSON="$(_rt_fetch_cache_summary 2>/dev/null)"; then
      ...
      if [[ "$(jq -r '.ok' <<<"$SUMMARY_JSON")" != "true" ]]; then
        ...
        read -r CHECKING ERROR DOWN STALLED_DL PAUSED_DL STOPPED_DL STALLED_UP UPLOADING STOPPED_UP TOTAL TRACKER_WARN TOP_STATES <<<"$(jq -r '
          [
